    ...
    while (nextUrl) {
      ...
      for (const entry of payload.value || []) {
        ...
        movies.push({
          ...
          // Prefer a real frame generated by OneDrive for files without a
          // downloaded title poster. This keeps thumbnails tied to the
          // uploaded video and avoids downloading the movie just to make art.
          posterUrl: videoThumbnailUrl || posterUrlFromTitle(title),
          size: entry.size || 0,
          source: "onedrive",
        });
      }
