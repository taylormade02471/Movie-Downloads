const { isStreamableExtension, movieTitleFromName, posterUrlFromTitle, hasBundledPoster } = require("../media");
const { buildSeriesGroups, inferSeriesInfo } = require("../library");
  ...
  async function listChildren(accessToken, itemId, folder = "") {
    ...
    while (nextUrl) {
      ...
      for (const entry of payload.value || []) {
        ...
        const videoThumbnailUrl = thumbnailSet?.large?.url
        ...
        movies.push({
          ...
          // Use a verified title poster before falling back to a OneDrive
          // frame. Frames can contain subtitles, orange progress bars, or
          // unrelated scenes and are not suitable as the normal card art.
          posterUrl: hasBundledPoster(title)
            ? posterUrlFromTitle(title)
            : videoThumbnailUrl || posterUrlFromTitle(title),
          size: entry.size || 0,
          source: "onedrive",
          ...seriesInfo,
        });
