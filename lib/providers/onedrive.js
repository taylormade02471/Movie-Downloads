...
function createOneDriveProvider({ env = process.env, fetchImpl = fetch, store }) {
  ...
  async function listMoviesWithAccessToken(accessToken) {
    const movies = await listChildren(accessToken, config.rootItemId);
  ...
  return {
    kind: "onedrive",
    async listMovies() {
      return withAccessTokenRetry((accessToken) => listMoviesWithAccessToken(accessToken));
    },
    async resolvePlayback(movieId) {
      return withAccessTokenRetry(async (accessToken) => {
        const movies = await listMoviesWithAccessToken(accessToken);
        ...
        if (!payload["@microsoft.graph.downloadUrl"]) {
          throw asProviderError("OneDrive did not return a playback URL.", 502);
        }

        return {
          url: payload["@microsoft.graph.downloadUrl"],
          expiresAt: null,
        };
      });
    },
